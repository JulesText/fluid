...
} elseif (isset($_REQUEST['listclear'])) {
    $action = 'listclear';
} elseif (isset($_REQUEST['ignoreclear'])) {
    $action = 'ignoreclear';
} elseif (isset($_REQUEST['allclear'])) {
    $action = 'listclear';
    $allclear = true;
} elseif (isset($_REQUEST['reset'])) {
    $action = 'reset';
} elseif (isset($_REQUEST['up_priorities'])) {
    $action = 'up_priorities';
} elseif (isset($_REQUEST['down_priorities'])) {
    $action = 'down_priorities';
} elseif (isset($_REQUEST['clearitemlists'])) {
    $action = 'clearitemlists';
}

if (isset($_POST['instanceId'])) {
    $values['instanceId'] = $_POST['instanceId'];
}

if (isset($_REQUEST['source'])) {
    $nextURL = "itemReport.php?itemId=" . $_REQUEST['itemId'];
}

if (isset($_REQUEST['matrix'])) {
    $nextURL = $_SERVER['HTTP_REFERER']; //"matrix.php";
}
...
switch ($action) {
    ...
    case 'itemdelete':
        ...
        if ($check) {
            $result = query("deletechecklistiteminst", $config, $values);
        }
    ...
    case 'listcreate':
        ...
        if ($check == 'check') {
            ...
            if (isset($_POST['scored']) && $_POST['scored'] == 'y') {
                $values['scored'] = 'y';
            } else {
                $values['scored'] = 'n';
            }
    ...
    case 'listdelete':
        ...
        $values['listId'] = $_REQUEST['listId'];
        if ($values['type'] !== 'c' && $values['type'] !== 'l') {
            die;
        }
