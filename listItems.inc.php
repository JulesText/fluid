...
if ($filter['timeframeId'] === '0') {
    $filter['timeframeId'] = null;
}
...
/* end of setting $filter
 --------------------------------------*/
if ($config['debug'] & _GTD_DEBUG) {
    echo '<pre>Filter:',print_r($filter, true),'</pre>';
}
...
foreach ($filter as $filterkey => $filtervalue) {
    if ($filtervalue != '') {
        $referrer .= "{$filterkey}={$filtervalue}&amp;";
    }
}
...
switch ($values['type']) {
    ...
    case "i":
        ...
        $show['dateCreated'] = false;
        $show['repeat'] = false;
        $show['assignType'] = true;
        $afterTypeChange = 'listItems.php?type=i';
        $checkchildren = false;
        break;
    // phpcs:ignore PSR2.ControlStructures.SwitchDeclaration.TerminatingComment -- Previous case terminates with break.
    case "cli":
        $typename = "Checklist Item";
        $parentname = "";
        $values['ptype'] = "";
        $show['parent'] = false;
        $show['behaviour'] = false;
        $show['category'] = false;
        $show['deadline'] = false;
    default:
        $typename = "Item";
...
if ($filter['everything'] != "true") {
    ...
    if ($filter['completed'] == "true") {
        $values['childfilterquery'] .= " AND " . sqlparts("completeditems", $config, $values);
    } else {
    ...
    if ($filter['repeatingonly'] == "true") {
        $values['childfilterquery'] .= " AND " . sqlparts("repeating", $config, $values);
    }

    if ($filter['dueonly'] == "true") {
        $values['childfilterquery'] .= " AND " . sqlparts("due", $config, $values);
    }
}
/*
Section Heading
...
if ($filter['needle'] !== '' && $filter['everything'] == "true") {
    ...
    if ($results_fi !== 0) {
        if ($result !== 0) {
            $result = array_merge($results_fi, $result);
        } else {
            $result = $results_fi;
        }
    }
...
if ($filter['needle'] !== '' && $filter['everything'] == "true") {
    ...
    if ($resultlists !== 0) {
        if ($result !== 0) {
            $result = array_merge($resultlists, $result);
        } else {
            $result = $resultlists;
        }
    }
...
if ($result) {
    ...
    foreach ($result as $row) {
        ...
        } elseif (in_array($row['type'], ['a', 'i', 's', 'r', 'w'])) {
            $maintable[$thisrow]['doreport'] = 'item';
        } else {
            $maintable[$thisrow]['doreport'] = $row['type']; # ['cl', 'cli', 'l', 'li', 'ai']
        }
...
*/
        ...
        if (isset($row['premiseA'])) {
            $descriptionString .= "<br><br>" . $row['premiseA'];
        }
        if (isset($row['premiseB'])) {
            $descriptionString .= "<br><br>" . $row['premiseB'];
        }
        if (isset($row['conclusion'])) {
            $descriptionString .= "<br><br>" . $row['conclusion'];
        }
        ...
        if (isset($row['standard']) && $row['standard'] != '') {
            $desiredOutcomeStr .= ", <br>" . $row['standard'];
        }
        if (isset($row['conditions']) && $row['conditions'] != '') {
        ...
        if (isset($childType) && count($childType)) {
            $maintable[$thisrow]['childtype'] = $childType[0];
        }
    ...
    if ($config['debug'] & _GTD_DEBUG) {
        echo '<pre>values to print:',print_r($maintable, true),'</pre>';
    }
} // end of: if($result)
/*
    ===================================================================
