...
$updateGlobals['referrer'] = $_SERVER['HTTP_REFERER']; //'matrix.php';
}
...
$updateGlobals['parents'] = array($updateGlobals['parents']);
}
...
if (isset($_POST['wasNAonEntry'])) {  // toggling next action status on several items
    ...
    if (isset($_POST['isNAs'])) {
$updateGlobals['isNA'] = $_POST['isNAs'];
    }
}
...
if ($updateGlobals['multi']) {
    ...
    if (isset($updateGlobals['wasNAonEntry']) && isset($updateGlobals['isNA'])) {  // toggling next action status on several items
        foreach (array_diff($updateGlobals['wasNAonEntry'], $updateGlobals['isNA']) as $values['itemId']) {
if ($values['itemId']) {
doAction('removeNA');
}
        }
        foreach (array_diff($updateGlobals['isNA'], $updateGlobals['wasNAonEntry']) as $values['itemId']) {
if ($values['itemId']) {
doAction('makeNA');
}
        }
...
$action = 'delete'; // override item-update if we are simply deleting
    }
    doAction($action);
}
...
function doAction($localAction)
{
 // do the current action on the current item; returns TRUE if succeeded, else returns FALSE
...
$title = 'item ' . $values['itemId'];
    ...
    if ($config['debug'] & _GTD_DEBUG) {
echo "<p><b>Action here is: $localAction item {$values['itemId']}</b></p>";
    }
    if ($config['debug'] & _GTD_FREEZEDB) {
return true;
    }
    switch ($localAction) {
        case 'makeNA':
...
$_POST = $_GET;
        ...
        if (!isset($_POST['type'])) {
die("$_POST[type] is null");
        }
        $sort['getitems'] = 'NULL';
        $values['filterquery'] = " WHERE ia.type = '" . $_POST['type'] . "' ";
        if (isset($_POST['categoryId'])) {
$values['filterquery'] .= " AND ia.categoryId = '" . $_POST['categoryId'] . "'";
        }
...
function deleteItem()
{
 // delete all references to a specific item
...
function createItem()
{
 // create an item and its parent-child relationships
    global $config,$values,$updateGlobals,$title;
    //Insert new records
...
function createItemQuickly()
{
// create an item when we only know its type and title - not yet in use - TOFIX still to check
    global $config,$values,$updateGlobals,$title;
    //Insert new records
    $result = query("newitem", $config, $values);
    $values['newitemId'] = $GLOBALS['lastinsertid'];
...
function updateItem()
{
 // update all the values for the current item
    global $config,$values,$updateGlobals,$title;
    query("deletelookup", $config, $values);
    removeNextAction();
    ...
    if ($values['type'] === $values['oldtype']) {
        setParents('update');
    } else { // changing item type - sever child links
        query("deletelookupparents", $config, $values);
    }
...
{
 // mark an item as completed, and recur if required
    global $config,$values;
...
$values['old'] = array();
    ...
    if ($values['repeat'] && empty($values['old']['dateCompleted'])) {
recurItem();
    } else {
makeComplete();
    }
...
{
 // mark the current item as a next action
    global $config,$values;
...
function removeNextAction()
{
 // remove the next action reference for the current item
    global $config,$values;
...
function retrieveFormVars()
{
    ...
    // key variables
    if (!empty($_POST['oldtype'])) {
        $values['oldtype'] = $_POST['oldtype'];
    }
...
{
 // retrieve all the values for the current item, and store in the $values array
    global $config,$values,$updateGlobals;
    $copyresult = query("selectitem", $config, $values, array());
    foreach ($copyresult[0] as $key => $thisvalue) {
...
function setParents($new)
{
    global $config,$values,$updateGlobals;
    if ($config['debug'] & _GTD_DEBUG) {
echo '<pre>',print_r($updateGlobals['parents'], true),'</pre>';
    }
    $markedna = false;
...
function recurItem()
{
 // mark a recurring item completed, and set up the recurrence
    global $config,$values,$updateGlobals;
...
{
 // mark an action as completed, and removes next action marker for it
    global $config,$values;
    query("completeitem", $config, $values);
    removeNextAction();
}
...
function nextPage()
{
 // set up the forwarding to the next page
...
$tst = 'list';
    ...
    switch ($tst) {
        case "another":
            ...
            foreach ($queryParameters as $key => $cat) {
                if (!empty($values[$key]) && $values[$key] != 'NULL') {
                    $nextURL .= "&amp;$cat=" . str_replace("'", "", $values[$key]);
                }
            }
            break;
        case 'child':
...
function literaldump($varname)
{
 // dump a variable name, and its contents
