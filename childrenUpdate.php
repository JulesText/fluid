...
if (!isset($_GET['categoryId'])) {
    $_GET['categoryId'] = false;
}
if (isset($_GET['catMultiId'])) {
    $qry = " OR ia.`categoryId` = '' OR ia.`categoryId` = '" . $_GET["catMultiId"] . "'";
} else {
    $qry = '';
}
...
if ($_GET['categoryId']) {
    $values['filterquery'] .= " AND (ia.categoryId = '" . $_GET['categoryId'] . "'" . $qry . ")";
}
...
*/ ?>
   ...
   <form action='processItems.php' method='post'>
    ...
    <div class='formbuttons'>
        ...
        <?php if ($_REQUEST['categoryId']) {
            echo "<input type='hidden' name='categoryId' value='" . $_REQUEST['categoryId'] . "' />";
        } ?>
        <?php if ($_REQUEST['matrix']) {
            echo "<input type='hidden' name='matrix' value='true' />";
        } ?>
        <?php if ($_REQUEST['visId'] > 0) {
            echo "<input type='hidden' name='visId' value='" . $_REQUEST['visId'] . "' />";
        } ?>
