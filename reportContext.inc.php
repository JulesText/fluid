...
foreach ($contextNames as $cid => $dummy1) {
    ...
    foreach ($result as $row) {
        if ((int) $row['contextId'] === $cid && $row['NA']) {
            $contextCount++;
        }
    }
    if ($contextCount) {
        $randomChoice = rand(1, $contextCount);
    } else {
        $randomChoice = 0;
    }
    $runningCount = 0;
    ...
    foreach ($timeframeNames as $tid => $dummy2) {
        ...
        while (
            ...
            if ($row['NA']) {
                array_push($wasNAonEntry[$cid][$tid], $row['itemId']);
            }
            $runningCount++;
...
if (count($lostitems)) {
    ...
    $tid = 0;
    $wasNAonEntry[$cid][$tid] = array();
    foreach ($timeframeNames as $thistid => $dummy1) {
        $matrixcount[$cid][$thistid] = 0;
    }
