...
// we've cancelled an over-write or a delete, so go back to the installation menu
    ...
    foreach ($_POST as $thiskey => $thisval) {
        if (_DEBUG) {
            echo "<p class='debug'>Is $thiskey is a delete key? ";
        }
...
function checkInstall()
{
    ...
    $checkState = 'config';
    if (_DEBUG) {
        echo '<p class="debug">Got config.php:</p><pre>',print_r($config, true),'</pre>';
    }
    if (!isset($config['db'])) {
    ...
    if (!checkPrefix($config['prefix'])) {
        exit(); // invalid prefix = fatal error
    }
    ...
    $nt = count($tablelist);
    if (_DEBUG) {
        echo "<pre>Number of tables: $nt<br />",print_r($tablelist, true),"</pre>";
    }
    ...
    */
    ...
    foreach ($gotPrefixes as $thisPreference) {
        ...
        if ($thisPrefix == $config['prefix']) { // we have an installation already using our target prefix
            ...
            } elseif (_ALLOWUPGRADEINPLACE || $versions[$thisVer]['database'] === $versions[_GTD_VERSION]['database']) {
        ...
        if ($config['prefix'] == '') { // prefixes weren't used in 0.8rc-1, so a blank target prefix means we are trying to upgrade in place, over the top of 0.8rc-1
            if (_ALLOWUPGRADEINPLACE) {
                ...
                if (_ALLOWUNINSTALL) {
                    ...
                    showNoUpgradeMsg($config['prefix'], '0.8rc-1');
                }
            }
        } else {
            $gotVersions['0.8rc-1='] = '0.8rc-1';
        }
    }
    ...
    if (!$destInUse && checkTables('0.7')) {
        ...
        if (count($tmptables)) {
            ...
            if (_ALLOWUNINSTALL) {
                ...
                echo "<p class='warning'>$msg<br />Change the installation prefix in config.php, or consult your administrator, to fix the problem.</p>\n";
            }
        }
    }
    ...
    if (_DEBUG) {
        echo '<pre>Versions found: ',print_r($gotVersions, true),"</pre>\n";
    }
    ...
    if ($goodToGo) {
        echo '<form action="install.php" method="post">'
            ...
            ,"</h3>\n";
        if (($destInUse || _ALLOWUNINSTALL) && count($gotVersions)) {
            showDeleteWarning();
        }
        echo "<table summary='table of installation alternatives'>\n"
            ,"<thead><tr><th>Use</th><th>From</th>\n";
        if (_DEBUG) {
            echo "<th class='debug'>name</th>\n";
        }
        if (_ALLOWUNINSTALL && count($gotVersions)) {
            echo "<th class='warning'>Press to delete: no installation will be done; the only action that will be taken is the removal of tables</th>\n";
        }
        echo "</tr></thead><tbody>\n";
...
function doInstall($installType, $fromPrefix)
{
    ...
    if (_DEBUG) {
        echo "<pre>Install type is: $installType<br />Source database has prefix $fromPrefix</pre>";
    }
