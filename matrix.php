...
if (strpos($url, 'matrix.php?') === false) {
    $url = str_replace('matrix.php', 'matrix.php?', $url);
}

// limit qualities
if (isset($_GET['qLimit'])) {
    $qLimit = $_GET['qLimit'];
} else {
    $qLimit = 'a';
}
...
// limit display
if (isset($_GET['nometa']) && $_GET['nometa'] == true) {
    $meta = false;
} else {
    $meta = true;
}
if (isset($_GET['scen']) && $_GET['scen'] == true) {
    $scen = true;
} else {
    $scen = false;
}
if (isset($_GET['calc']) && $_GET['calc'] == true) {
    $calc = 'true';
} else {
    $calc = 'false';
}
...
if (isset($_GET['career']) && $_GET['career'] == true) {
    $career = true;
} else {
    $career = false;
}
if (isset($_GET['travel']) && $_GET['travel'] == true) {
    $travel = true;
} else {
    $travel = false;
}
...
if (isset($_GET['visions_hide']) && $_GET['visions_hide'] == true) {
    $visions_hide = true;
} else {
    $visions_hide = false;
}
...
foreach ((array) $attrStat as $attr) {
    if (isset($attr['existTable'])) {
        $attrStat[$i]['existTable'] = unserialize($attr['existTable']); // values in other db tables
    }
    if (isset($attr['typeNoEd'])) {
        $nonEdit[] = array('qId' => $attr['qId'], 'typeNoEd' => $attr['typeNoEd']); // non-editable attributes
    }
    $i++;
}
...
$tLimit = '';
foreach ((array) $attrStat as $attr) {
    if (!empty($attr) && !empty($attr['format']) && $attr['format'] == 'unqtimeline') {
        $tLimit .= $attr['qId'] . ',';
    }
}
...
if (is_array($attrVar)) { # for some displays like Career disp = e, we expect no result for $attrVar query
    foreach ((array) $attrVar as $row) {
        if ($row['format'] == 'unqhoursyear') {
            $unqhoursyear = $row['title'];
        }
        if ($row['format'] == 'unqhoursyearbrainless') {
            $unqhoursyearbrainless = $row['title'];
        }
        if ($row['format'] == 'unqcorrelpref') {
            $unqcorrelpref = $row['title'];
        }
        if ($row['format'] == 'unqcorrelbala') {
            $unqcorrelbala = $row['title'];
        }
    }
}
...
} elseif ($vLimit) { // vLimit in request for only 1 vision
    $values = array();
    if (!isset($values['filterquery'])) {
        $values['filterquery'] = '';
    }
...
if ($test) {
    $values['filterquery'] .= ' AND i.title = "a test" ';
}
if ($live) {
    $values['filterquery'] .= ' AND ia.isSomeday = "n" AND its.dateCompleted IS NULL ';
}
if ($vLimit) {
    $values['filterquery'] .= ' AND i.itemId = ' . $vLimit;
}
...
foreach ((array) $vis as $visn) {
    ...
    if (!is_array($result)) {
        continue; // vision no children, skip query, avoid error from non array result
    }
...
//echo '<pre>'; var_dump($row['dateCompleted']);
        ...
        $maintable[$i]['items'] =
                    ...
                    '<a href="itemReport.php?itemId=' . $row['itemId'] . '" target="_blank" class="mx" title="edit">E</a>&nbsp;' .
...
foreach ((array) $maintable as $row) {
    ...
    if ($row['type'] == 'v') {
        ...
        if (!$data) {
            $maintemp[]['row.class'] = 'blank v ' . $visId; //blank row
        }
    }
}
...
$(document).ready(function() {
    ...
    if ($live) {
        echo "toggleCheckB('someday');toggleCheckB('complete');\n\t";
    }
    if ($career) {
        echo "toggleCheckB('attr.27');\n\t";
    }
    if ($travel) {
        echo "toggleCheckB('attr.26');\n\t";
    }
    if ($compare) {
        echo "toggleCheckB('attr.24');\n\t";
    }
    if (!$meta) {
        echo "toggleCol(['3','4','5']);\n\t";
    }
    if ($visions_hide) {
        echo "toggleRow(['vision','blank']);\n\t";
    }
    ...
    if ($qLimit == 'j') {
        ...
        foreach ((array) $valIds as $valId => $name) {
            ...
            foreach ((array) $result as $r) {
                $rName = $r['title'];
                $rDisp = str_replace($needle, '', $r['disp']);
            }
            $valIds[$valId] = $rName;
...
<table class="matrixCont">
    ...
    foreach ((array) $angles as $vala) {
        ...
        foreach ((array) $vala['qualities'] as $valN) {
            $comma = '';
            echo "<td class='cont";
            if (array_search($valN['title'], $quals) > -1) {
                echo " italic";
            }
