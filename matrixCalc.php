...
if (isset($_POST["qLimit"])) {
    $qLimit = $_POST["qLimit"];
} else {
    $qLimit = 'a';
}
if (isset($_POST["vLimit"]) && $_POST["vLimit"] != '') {
    $vLimit = $_POST["vLimit"];
} else {
    $vLimit = false;
}
if (isset($_POST["debugSave"]) && $_POST["debugSave"] != '') {
    $debugSave = $_POST["debugSave"];
} else {
    $debugSave = false;
}
...
if ($vLimit) {
    $values['filterquery'] .= ' AND ia.itemId IN (' . $vLimit . ') ';
}
...
foreach ((array) $attributes as $attr) {
    ...
    if ($attr['format'] == 'unqhourstravel') {
        $travelId = $attr['qId'];
        $unqhoursIds[] = $attr['qId'];
    }

    // attr: Effort / Year (int)
    if ($attr['format'] == 'unqhours') {
        $hoursId = $attr['qId'];
        $unqhoursIds[] = $attr['qId'];
    }

    // attr: Hours research (int)
    if ($attr['format'] == 'unqhoursresearch') {
        $reseId = $attr['qId'];
        $unqhoursIds[] = $attr['qId'];
    }

    // attr: Years (int)
    if ($attr['format'] == 'unqyears') {
        $yearqId = $attr['qId'];
    }

    // attr: Years probability (int)
    if ($attr['format'] == 'unqyearsprob') {
        $yearsprobId = $attr['qId'];
    }

    // attr: Brainless hours (boolean)
    if ($attr['format'] == 'unqbrainless') {
        $brainlessId = $attr['qId'];
    }

    // attr: [one of the n years duration of timeline] (int)
    if ($attr['format'] == 'unqtimeline') {
        $unqtimelineIds[$attr['title']] = $attr['qId'];
        $skipqIds[] = $attr['qId'];
    }

    // attr: Year Start (date)
    if ($attr['format'] == 'unqyearstart') {
        $yearstqId = $attr['qId'];
    }

    // attr: Year End (date)
    if ($attr['format'] == 'unqyearend') {
        $yearenqId = $attr['qId'];
    }

    // variable to flag optimise preferences attribute id
    if ($attr['format'] == 'unqoptimisepref') {
        $unqoptimiseprefId = $attr['qId'];
        $skipqIds[] = $attr['qId'];
        $optim = true;
    }

    // variable to flag optimise balance attribute id
    if ($attr['format'] == 'unqoptimisebala') {
        $unqoptimisebalaId = $attr['qId'];
        $skipqIds[] = $attr['qId'];
        $optim = true;
    }

    // variable to flag values sum attribute id
    if ($attr['format'] == 'unqvaluessum') {
        $unqvaluessumId = $attr['qId'];
        $skipqIds[] = $attr['qId'];
    }

    // attr: Value Sum / Hours (int)
    if ($attr['format'] == 'unqvaluessumhrs') {
        $unqtimelineIds[0] = $attr['qId'];
        $skipqIds[] = $attr['qId'];
    } // not clear why unqvaluessumhrs needs to be in $unqtimelineIds array

    // variable to flag contexts sum attribute id
    if ($attr['format'] == 'unqcontextssum') {
        $unqcontextssumId = $attr['qId'];
        $skipqIds[] = $attr['qId'];
    }

    // attr: Context Sum / Hours (int)
    if ($attr['format'] == 'unqcontextssumhrs') {
        $unqcontextssumhrs = $attr['qId'];
        $skipqIds[] = $attr['qId'];
    }
    ...
    if ($attr['format'] == 'score') {
        $scoreqIds[$attr['qId']] = $attr['weight'];
        $scoreweights[$attr['qId']] = null;
    }
    ...
    if ($attr['format'] == 'unqprobability') {
        $skipqIds[] = $attr['qId'];
        $unqprobabilityId = $attr['qId'];
    }

    // skip all probability items as they are currently called from the attribute itself
    if ($attr['format'] == 'probability') {
        $skipqIds[] = $attr['qId'];
    }

    // attr: Career (boolean)
    if ($attr['format'] == 'unqcareer') {
        $careerId = $attr['qId'];
    }
}
...
if ($optim) {
    ...
    foreach ((array) $visions as $visn) {
        $scoreopt[] = array('type' => 'v', 'visId' => $visn['itemId'], 'scores' => $scoreweights);
    }
}
...
foreach ((array) $qualMeta as $qualMet) {
    ...
    if ($qualMet['format'] == 'someday' && $qualMet['typeReq'] == 'cl') {
        $listSomedayId = $qualMet['qId'];
    }
}
...
// loop visions
foreach ((array) $visions as $visn) {
    if ($visn['isSomeday'] == 'n' && $visn['dateCompleted'] == '') {
        $active = 1;
    } else {
        $active = 0;
    }
...
foreach ((array) $attributes as $attr) {
    ...
    if (in_array($attr['qId'], $skipqIds, true)) {
        continue;
    }
    ...
    foreach ((array) $visions as $visn) {
        ...
        foreach ((array) $items as $item) {
            ...
            if ($item['value'] == 0 && $attr['qId'] != $hoursId) {
                if ($optim && array_key_exists($attr['qId'], $scoreqIds)) {
                    $scores[] = array('type' => $item['itemType'], 'visId' => $visn['itemId'], 'id' => $item['itemId'], 'attrId' => $attr['qId'], 'value' => $item['value']);
                }
                continue;
            }
            ...
            // else item is not a list
                ...
                if (!empty($itemPars) && is_array($itemPars) && count($itemPars) > 0) {
                    foreach ((array) $itemPars as $pars) {
                        if ($pars['parentId'] == $visn['itemId']) {
                            $child = true;
                        }
                    }
                }
            ...
            if (
                array_key_exists($attr['qId'], $scoreqIds) ||
            ...
            if (array_key_exists($attr['qId'], $scoreqIds)) {
                $scores[] = array('type' => $item['itemType'], 'visId' => $visn['itemId'], 'id' => $item['itemId'], 'attrId' => $attr['qId'], 'value' => $sum);
            }
            ...
            if (in_array($item['qId'], $unqhoursIds, true)) {
                ...
                */
                ...
                if ($yrend > count($unqtimelineIds) - 1) {
        ...
        if ($outputYrs > 0) {
            foreach ((array) $visYrs as $key => $visYr) {
                if ($visn['itemId'] == $key) {
                    if ($outputYrs > $visYr['yrs']) {
                        $visYrs[$key]['yrs'] = $outputYrs;
                    }
                    break;
                }
            }
    ...
    // if output obtained, or the current attribute must force a response to the matrix, push the record onto the final result
    if ($summary1 !== '' || in_array($attr['format'], $resForce)) {
        $result[] = array('type' => 'x', 'attrId' => $attr['qId'], 'form' => 'a', 'value' => $summary1);
    }
    if ($summary2 !== '' || in_array($attr['format'], $resForce)) {
        $result[] = array('type' => 'x', 'attrId' => $attr['qId'], 'form' => 'b', 'value' => $summary2);
    }
...
foreach ((array) $visYrs as $key => $visYr) {
    if (isset($yearqId)) {
        $result[] = array('type' => 'v', 'visId' => $key, 'attrId' => $yearqId, 'value' => date("Y") - $visYr['yearst'] + $visYr['yrs']);
    }
    if (isset($yearstqId) && $visYr['active']) {
        $result[] = array('type' => 'v', 'visId' => $key, 'attrId' => $yearstqId, 'value' => $visYr['yearst']);
    }
    if (isset($yearenqId) && $visYr['active']) {
        $result[] = array('type' => 'v', 'visId' => $key, 'attrId' => $yearenqId, 'value' => date("Y") + $visYr['yrs'] - 1);
    }
    if (!$visYr['active']) {
        continue;
    }
    if (!$summaryYear) {
        $summaryYear = $visYr['yearst'];
    }
    if ($summaryYrs < $visYr['yrs']) {
        $summaryYrs = $visYr['yrs'];
    }
    if ($summaryYear > $visYr['yearst'] && $visYr['active']) {
        $summaryYear = $visYr['yearst'];
    }
}
if (isset($yearqId)) {
    $result[] = array('type' => 'x', 'attrId' => $yearqId, 'form' => 'a', 'value' => $summaryYrs);
}
if (isset($yearqId)) {
    $result[] = array('type' => 'x', 'attrId' => $yearqId, 'form' => 'b', 'value' => '');
}
if (isset($yearstqId)) {
    $result[] = array('type' => 'x', 'attrId' => $yearstqId, 'form' => 'a', 'value' => $summaryYear);
}
if (isset($yearstqId)) {
    $result[] = array('type' => 'x', 'attrId' => $yearstqId, 'form' => 'b', 'value' => '');
}
if (isset($yearenqId)) {
    $result[] = array('type' => 'x', 'attrId' => $yearenqId, 'form' => 'a', 'value' => $summaryYrs + date("Y") - 1);
}
if (isset($yearenqId)) {
    $result[] = array('type' => 'x', 'attrId' => $yearenqId, 'form' => 'b', 'value' => '');
}
...
if ($optim) {
    ...
    foreach ((array) $scores as $score) {
        ...
        if (!$exist) {
            $scoreopt[] = array('type' => $score['type'], 'visId' => $score['visId'], 'id' => $score['id'], 'scores' => $scoreweights);
        }
    ...
    foreach ((array) $scoreopt as $opt) {
        ...
        // push to result
        if ($opt['type'] == 'x') {
            if (isset($unqoptimiseprefId)) {
                $result[] = array('type' => 'x', 'attrId' => $unqoptimiseprefId, 'form' => $opt['form'], 'value' => $pearson);
            }
            if (isset($unqoptimisebalaId)) {
                $result[] = array('type' => 'x', 'attrId' => $unqoptimisebalaId, 'form' => $opt['form'], 'value' => $uniformity);
            }
        } elseif ($opt['type'] == 'v') {
            if (isset($unqoptimiseprefId)) {
                $result[] = array('type' => 'v', 'visId' => $opt['visId'], 'attrId' => $unqoptimiseprefId, 'value' => $pearson);
            }
            if (isset($unqoptimisebalaId)) {
                $result[] = array('type' => 'v', 'visId' => $opt['visId'], 'attrId' => $unqoptimisebalaId, 'value' => $uniformity);
            }
        } else {
            if (isset($unqoptimiseprefId)) {
                $result[] = array('type' => $opt['type'], 'visId' => $opt['visId'], 'itemId' => $opt['id'], 'attrId' => $unqoptimiseprefId, 'value' => $pearson);
            }
            if (isset($unqoptimisebalaId)) {
                $result[] = array('type' => $opt['type'], 'visId' => $opt['visId'], 'itemId' => $opt['id'], 'attrId' => $unqoptimisebalaId, 'value' => $uniformity);
            }
        }
    }
}
...
if (isset($unqtimelineIds[0])) {
    foreach ((array) $visions as $visn) {
        foreach ((array) $valuessumvishours as $key => $sumv) {
            if ($visn['itemId'] == $key) {
                if ($sumv['hours'] <= 0) {
                    $sumv['hours'] = 1; // error handling if hours for item = 0 or negative, assume no division
                }
...
foreach ((array) $contextssum as $contextsum) {
    ...
    if ($contextsum['active']) {
        $contextssumvis[$contextsum['visId']] += $contextsum['value'];
    }
}
...
foreach ((array) $timeline as $tline) {
    ...
    if (!$tline['active'] || $tline['attrId'] == $unqtimelineIds[0]) {
        continue;
    }
    // search visions
            ...
            foreach ((array) $varray as $tlineId => $tarray) {
                ...
                if ($tlineId == $tline['attrId']) {
                    ...
                    foreach ((array) $tarray as $tlinepair => $value) {
                        ...
                        if ($tlinepair == 'a') {
                            $tlinevis[$visId][$tlineId][$tlinepair] = $value + $tline['valuea'];
                        }
                        if ($tlinepair == 'b') {
                            $tlinevis[$visId][$tlineId][$tlinepair] = $value + $tline['valueb'];
                        }
                        $i++;
                        if ($i > 2) {
                            break 3;
...
foreach ((array) $timeline as $tline) {
    ...
    if ($tline['valuea'] == '' && $tline['valueb'] == '') {
        $output = '';
    } else {
        $output = $tline['valuea'] + $tline['valueb'];
    }
/*
...
foreach ((array) $tlineNull as $ren) {
    foreach ((array) $unqtimelineIds as $attrId) {
        ...
        if (!$match) {
            $result[] = array('type' => $ren['type'], 'visId' => $ren['visId'], 'itemId' => $ren['itemId'], 'attrId' => $attrId, 'value' => '');
        }
    }
}
...
foreach ((array) $certaintiesitem as $certainty) {
    ...
    if (floor($value) > $certaintyMax) {
        $certaintyMax = floor($value);
    }
    if (floor($value) < $certaintyMin) {
        $certaintyMin = floor($value);
    }
...
$certaintyviscount = 0;
foreach ((array) $visions as $visn) {
    if ($certaintiesvis[$visn['itemId']]['count'] == 0) {
        break;
    }
...
// crude error catch here, maybe should not be calculating to begin with unless called in qLimit?
if (isset($unqprobabilityId)) {
    if ($certaintyviscount > 0) {
        $result[] = array('type' => 'x', 'attrId' => $unqprobabilityId, 'form' => 'a', 'value' => $certaintyMax);
    }
    if ($certaintyviscount > 0) {
        $result[] = array('type' => 'x', 'attrId' => $unqprobabilityId, 'form' => 'b', 'value' => $certaintyMin);
    }
...
function uniformity($x)
{
    ...
    if (empty($x) || count($x) == 0 || array_sum($x) == 0) {
        return 0;
    }
