...
/*
   ======================================================================================
*/
function recreateNextactions($prefix)
{
 // recreate the nextactions table, removing all inconsistencies
...
function checkPrefix($prefix)
{
    ...
    if (_DEBUG) {
        echo '<p class="debug">Validating prefix ' . "'{$prefix}'</p>\n";
    }
...
function checkPrefixedTables($prefix)
{
    global $versions;
    if (_DEBUG) {
        echo '<p class="debug">Is there a current 0.8 installation with prefix "',$prefix,'"? ';
    }
    ...
    if (_DEBUG) {
        echo (($doneOK) ? 'YES' : 'NO'),"</p><p class='debug'>Resolved version number as: '$retval'</p>\n";
    }
    return $retval;
}
/*
...
function checkVersion($prefix)
{
    ...
    if (empty($result)) {
        ...
        $retval = $last[0];
        if (_DEBUG) {
            echo "<p class'debug'>Found Version field: $retval</p>";
        }
    }
...
function getExistingDestinationTables($prefix)
{
    ...
    if (_DEBUG) {
        echo '<p class="debug">Checking availability of destination prefix "{$prefix}"</p>';
    }
    $destInUse = array();
...
function failDuringCheck()
{
    ...
    switch ($checkState) {
        case 'ok':
            return; // reached end ok, so nothing to do
