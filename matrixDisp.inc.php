<thead class="mx">
    ...
    if ($angles) {
        ...
        foreach ((array) $angles as $val) {
            ...
            while ($trans > 0) {
                $anglesStr .= "<th class='{$val['qType']} {$val['qId']} trans'></th>";
                $trans--;
            }
        }
        $anglesStr .= "\n</tr>\n";
        ...
        foreach ((array) $angles as $val) {
            foreach ((array) $val['qualities'] as $valN) {
                foreach ((array) $valN['attributes'] as $valO) {
                    $attrStr .= "<th class='attr {$valO['qId']} b mx {$valO['style']}'>&nbsp;</th>\n";
                }
            }
        }
        $attrStr .= "</tr>\n";
        ...
        foreach ((array) $angles as $val) {
            foreach ((array) $val['qualities'] as $valN) {
                foreach ((array) $valN['attributes'] as $valO) {
                    ...
                    if (isset($valO['weight'])) {
                        $valO['description'] .= " (Weight = " . $valO['weight'] . "/9)";
                    }
                    $attrTit .= "<th class='attr {$valO['qId']} mx {$valO['style']}' title='{$valO['description']}'>{$title}</th>\n";
                    if ($valO['title'] == '') {
                        echo '<div style="position: relative; z-index: 10000;">disp mismatch attr ' . $valN['qId'] . ' (check qualities > "disp" var)</div>';
                    }
                }
    ...
    foreach ((array) $dispArray as $key => $val) {
        ...
        // if this attribute calls from existing table, and not for this item type, will the value be non editable? default is no
                ...
                foreach ((array) $attrStat as $attr) {
                    if ($attr['qId'] == $qId && !is_null($attr['existTable'])) {
                        foreach ((array) $attr['existTable'] as $crit) {
                            if (isset($crit['limit']) && $crit['limit'] == 1) {
                                $existOnly = true;
                            }
                            ...
                            if (strpos($crit['type'], 'z') > -1) {
                                ...
                                if (isset($crit['keyTrim'])) {
                                    $existXTrim = $crit['keyTrim'];
                                } else {
                                    $existXTrim = false;
                                }
                                $existXRef = true;
                        ...
                        if ($existXRef) {
                            ...
                            if ($existXTrim) {
                                $existKeyVal = substr($existKeyVal, 0, $existXTrim); // if only limited variable
                            }
            ...
            if ($key == 'toggle' && $row['type'] == 'v') {
                ...
                $saveId = $row[$existKey];
                if (in_array($row["type"], array('c','l'))) {
                    $saveId = $row['itemId'];
                }
                if (is_numeric($saveId)) {
                    echo $editable . ' onBlur="sT(this,\'' . $existTable . '\',\'' . $existField . '\',\'' . $existKey . '\',\'' . $saveId . '\')" onFocus="sE(this)"';
                }
            } elseif (
                isset($row[$key]) &&
                strpos($key, 'attr') > -1 &&
            ...
            if (!$checkbox) {
                if ($key == 'item' && $data) {
                    $row[$key] = preg_replace('/[^A-Za-z0-9 ]/', '', $row[$key]);
                }
                ...
                if ($mxlink && strlen($tdstr) > 0) {
                    $tdstr = faLink($tdstr, true);
                }
            }
            if ($data) {
                if (strpos($tdstr, 'checkbox') !== false) {
                    if (strpos($tdstr, 'checked') === false) {
                        $tdstr = 0;
                    } else {
                        $tdstr = 1;
                    }
