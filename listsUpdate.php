...
$values['filterquery'] = "";
if ($_GET['catMultiId']) {
    $qry = "l.`categoryId` = '' OR l.`categoryId` = '" . $_GET["catMultiId"] . "' OR ";
} else {
    $qry = '';
}
if ($values['categoryId']) {
    $values['filterquery'] = " WHERE " . $qry . sqlparts("listcategoryfilter", $config, $values);
}
...
if ($_GET['visId'] > 0 && $_GET['visId'] != $_GET['itemId']) {
    ...
    if (is_array($resultV) && count($resultV) > 0) {
        foreach ((array) $resultV as $l) {
            array_push($highlights, $l['listId']);
        }
    }
# is a vision
...
<?php if ($result) {
    ?>
    <form action='processLists.php<?php if ($_GET['matrix']) {
        echo '?matrix=true';
                                  } ?>' method='post'>
    <?php $tstr = '
            ...
            <tr>
                ...
                if (is_array($resultListX)) {
                    foreach ($resultListX as $resultListN) {
                        if ($resultListN['listId'] == $row['listId']) {
                            echo 'checked="checked"';
                        }
                    }
                }

                if (in_array($row['listId'], $highlights)) {
                    echo ' style="outline: 3px solid #aca;"';
                }
    ...
    <div class='formbuttons'>
        ...
        <?php if ($_GET['visId'] > 0) {
            echo "<input type='hidden' name='visId' value='" . $_GET['visId'] . "' />";
        } ?>
