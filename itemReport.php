...
if (isset($_GET['content']) && $_GET['content'] == "limit") {
    $urltoken = "&content=limit";
} else {
    $urltoken = "";
}
if (isset($_GET['showCompleted']) && $_GET['showCompleted'] == "true") {
    $urltoken .= "&showCompleted=true";
}
if (!isset($_GET['content'])) {
    echo "&nbsp;&nbsp;&nbsp;[&nbsp;<a href=\"itemReport.php?itemId={$values['itemId']}&content=limit{$urltoken}\">Limit</a>&nbsp;]";
}
...
if ($item['type'] === 'i') {
    echo "[<a href='assignType.php?itemId={$values['itemId']}&amp;referrer=$afterTypeChange'>Set type</a>] \n";
}
/*
...
//Item details
if ($item['description']) {
    echo "<tr><th width=200>Description:</th><td class=\"JKPadding\">" . nl2br(escapeChars($item['description'])) . "</td></tr>\n";
}
if (strlen($item['conclusion']) > 1) {
    echo "<tr><th>Reasons:</th><td class=\"JKPadding\">";
    if ($item['premiseA']) {
        echo "<div contenteditable='true'" . ajaxUpd('itemPremiseA', $item['itemId']) . ">" . $item['premiseA'] . "</div>";
    }
    if ($item['premiseB']) {
        echo "<br><div contenteditable='true'" . ajaxUpd('itemPremiseB', $item['itemId']) . ">" . $item['premiseB'] . "</div>";
    }
    if ($item['conclusion']) {
        echo "<br><div contenteditable='true'" . ajaxUpd('itemConclusion', $item['itemId']) . ">" . $item['conclusion'] . "</div>";
    }
    echo "</td></tr>\n";
}
if (strlen($item['behaviour']) > 1) {
    ...
    if ($item['behaviour']) {
        echo "<div contenteditable='true'" . ajaxUpd('itemBehaviour', $item['itemId']) . ">" . $item['behaviour'] . "</div>";
    }
    if ($item['standard']) {
        echo "<br><div contenteditable='true'" . ajaxUpd('itemStandard', $item['itemId']) . ">" . $item['standard'] . "</div>";
    }
    if ($item['conditions']) {
        echo "<br><div contenteditable='true'" . ajaxUpd('itemConditions', $item['itemId']) . ">" . $item['conditions'] . "</div>";
    }
    echo "</td></tr>\n";
}
if ($item['hyperlink']) {
...
if ($item['categoryId']) {
    echo "<tr><th>Category:</th><td><a href='editCat.php?id={$item['categoryId']}&amp;field=category'>" . makeclean($item['category']) . "</a></td></tr>\n";
}
if ($item['contextId']) {
    echo "<tr><th>Space Context:</th><td><a href='editCat.php?id={$item['contextId']}&amp;field=context'>" . makeclean($item['cname']) . "</a></td></tr>\n";
}
if ($item['timeframeId']) {
    echo "<tr><th>Time Context:</th><td><a href='editCat.php?id={$item['timeframeId']}&amp;field=time-context'>" . makeclean($item['timeframe']) . "</a></td></tr>\n";
}
...
if ($item['type'] === 'a' || $item['type'] === 'w') {
    echo '<tr><th>Next Action?</th><td>',($item['NA']) ? 'Yes' : 'No',"</td></tr>\n";
}
if ($item['repeat']) {
    echo '<tr><th>Repeat every</th><td>' . $item['repeat'] . ' days' . "</td></tr>\n";
}

if ($showCompleted) {
    echo '<tr><th>Created:</th><td>' . $item['dateCreated'] . "</td></tr>\n";
    if ($item['lastModified']) {
        echo '<tr><th>Last modified:</th><td>' . substr($item['lastModified'], 0, 10) . "</td></tr>\n";
    }
    ...
    <div class='reportsection'>
            ...
            if ($comp === "y") {
                ...
                foreach (array('categoryId','contextId','deadline','isTrade') as $field) {
                    if ($item[$field]) {
                        $createURL .= "&amp;$field={$item[$field]}";
                    }
                }
                if ($values['isSomeday'] != 'y') {
                    $title = "<a href='$createURL' title='Add new " . $typename[$thistype] . "'>Add " . $title . "</a>";
                }
            }
        ...
        //$dispArray[$outcomeField]='Objective';
            ...
            if ($config['nextaction'] === 'single') {
                $dispArray['NA.type'] = 'radio';
            }
            $i = 0;
            $maintable = array();
            ...
            foreach ($result as $row) {
                ...
                if (strstr($row['description'], '<a href=')) {
                ...
                } elseif (in_array($row['type'], ['v','o','g','p'])) {
                  ...
                  // if ($row['premiseB']) $maintable[$i][$descriptionField] .= $row['premiseB'] . '<br><br>';
                    if ($row['conclusion']) {
                        $maintable[$i][$descriptionField] .= $row['conclusion'] . '<br><br>';
                    }
                    $maintable[$i][$descriptionField] .= '<div>';
                } elseif ($isTrade == 'y' || strlen($row['conclusion']) > 1) {
                    $maintable[$i][$descriptionField] = '';
                    if (in_array($row['tradeConditionId'], ['',0,1,null])) {
                        $maintable[$i][$descriptionField] .= "<div contenteditable='true'" . ajaxUpd('itemDescription', $row['itemId']) . ">" . $row['description'] . "</div>";
                    }
                    if (
                        !in_array($row['tradeConditionId'], ['',0,null])
                        && (strlen($row['premiseA']) > 0 || strlen($row['premiseB']) > 0 || strlen($row['conclusion']) > 0)
                    ) {
                        if (in_array($row['tradeConditionId'], [1])) {
                            $maintable[$i][$descriptionField] .= PHP_EOL;
                        }
                ...
                if ($row['hyperlink']) {
                    if (!empty($row['description'])) {
                        $maintable[$i][$descriptionField] .= "</div><div><br>";
                    } else {
                        $maintable[$i][$descriptionField] .= "</div><div>";
                    }
                ...
                $rfield = $row['premiseA'];
                if ($row['premiseB']) {
                    $rfield .= '<br><br>' . $row['premiseB'];
                }
                if ($row['conclusion']) {
                    $rfield .= '<br><br>' . $row['conclusion'];
                }
            ...
            if ($isTrade == 'y') {
                ...
                foreach ($index as &$element) {
                    ...
                    foreach ($maintable as $row) {
                        if (
                            $element['dateCreated'] == $row['dateCreated']
                            ...
                            if ($row['tradeCondition'] == 'Valuation*') {
                                $element['condition1'] = true;
                            }
                            if ($row['tradeCondition'] == 'Valuation*' && $row['deadline'] != '') {
                                $element['condition2'] = true;
                            }
                            if ($row['tradeCondition'] == 'Time Lapse*') {
                                $element['condition3'] = true;
                            }
                            if ($row['tradeCondition'] == 'Time Lapse*' && $row['deadline'] != '') {
                                $element['condition4'] = true;
                            }
                            if ($row['tradeCondition'] == 'Stop Loss*') {
                                $element['condition5'] = true;
                            }
                            if ($row['tradeCondition'] == 'Take Profit*') {
                                $element['condition6'] = true;
                            }
                        }
                    }
                }
                ...
                foreach ($maintable as &$row) {
                        ...
                        if ($element['dateCreated'] == $row['dateCreated'] && $element['title'] == $row['title']) {
                            ...
                            if ($row['isStrategy']) {
                                ...
                                if (!$element['condition1']) {
                                    $row['valuation'] .= ' Valuation* condition missing;';
                                } elseif (!$element['condition2']) {
                                    $row['valuation'] .= ' Valuation* missing due date;';
                                }
                                if (!$element['condition3']) {
                                    $row['valuation'] .= ' Time Lapse* condition missing;';
                                } elseif (!$element['condition4']) {
                                    $row['valuation'] .= ' Time Lapse* missing due date;';
                                }
                                if (!$element['condition5']) {
                                    $row['valuation'] .= ' Stop Loss* condition missing;';
                                }
                                if (!$element['condition1']) {
                                    $row['valuation'] .= ' Take Profit* condition missing;';
                                }
                                if ($row['tradeCondition'] == 'Valuation*') {
                    ...
                    if ($row['tradeCondition'] == '') {
                        $row[$outcomeField] =
                        $row['valuation'];
                    } elseif ($row['tradeCondition'] == 'Valuation*' && $row['isTrade'] == 'y') {
                        $row[$outcomeField] =
                        "<u>" . $row['tradeCondition'] . "</u>" . PHP_EOL
                        . 'Valuation: ' . $row['valuation'];
                    } elseif ($row['tradeCondition'] == 'Valuation*') {
                        $row[$outcomeField] =
                        "<u>" . $row['tradeCondition'] . "</u>";
                    } elseif ($row['tradeCondition'] != '') {
                        $row[$outcomeField] =
                        "<u>" . $row['tradeCondition'] . "</u>"
