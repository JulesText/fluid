...
<div id="filter">
    <form action="listItems.php" method="post">
        <div class="formrow">
            ...
            <input type="checkbox" id="notcategory" name="notcategory" title="Exclude category from list" value="true" <?php if ($filter['notcategory'] == "true") {
            ...
            <input type="checkbox" id="notspacecontext" name="notspacecontext" title="Exclude spatial context from list" value="true" <?php if ($filter['notspacecontext'] == "true") {
                echo 'checked="checked"';
                                                                                                                                      }?> />
            ...
            <input type="checkbox" name="nottimecontext" id="nottimecontext" title="Exclude time context from list" value="true" <?php if ($filter['nottimecontext'] == "true") {
                echo 'checked="checked"';
                                                                                                                                 }?> />
            <label for='nottimecontext' class='notfirst'>NOT</label>
        </div>
        <div class="formrow">
            <input type='checkbox' name='completed' id='completed' value='true' class="left first" <?php if ($filter['completed'] == "true") {
                echo 'checked="checked"';
                                                                                                   }?> title="Show achievements" /><label for='completed' class='right'>Completed</label>
            <input type='checkbox' name='tickler' id='suppressed' value='true' class="notfirst" <?php if ($filter['tickler'] == "true") {
                echo 'checked="checked"';
                                                                                                }?> title="Show tickler <?php echo $typename ?>s" /><label for='suppressed' class='right'>Tickler</label>
            <input type='checkbox' name='someday' id='someday' value='true' class="notfirst" <?php if ($filter['someday'] == "true") {
                echo 'checked="checked"';
                                                                                             }?> title="Show someday/maybe <?php echo $typename ?>s" /><label for='suppressed' class='right'>Someday</label>
            <input type="checkbox" name="nextonly" id="nextonly" class="notfirst" value="true" <?php if ($filter['nextonly'] == "true") {
                echo 'checked="checked"';
                                                                                               }?> title="Show only Next Actions" /><label for='nextonly' class='right'>Next Actions</label>
            <input type="checkbox" name="dueonly" id="dueonly" class="notfirst" value="true" <?php if ($filter['dueonly'] == "true") {
                echo 'checked="checked"';
                                                                                             }?> title="Show only <?php echo $typename ?>s with a due date" /><label for='dueonly' class='right'>Due</label>
            <input type="checkbox" name="repeatingonly" id="repeatingonly" class="notfirst" value="true" <?php if ($filter['repeatingonly'] == "true") {
                echo 'checked="checked"';
                                                                                                         }?> title="Show only repeating <?php echo $typename ?>s" /><label for='repeatingonly' class='right'>Repeating</label>
            <input type="checkbox" name="everything" id="everything" class="notfirst" value="true" <?php if ($filter['everything'] == "true") {
                echo 'checked="checked"';
                                                                                                   }?> title="Show all <?php echo $typename ?>s, regardless of status or labels" onclick='javascript:filtertoggle("toggle");' /><label for='everything'>Show all</label>
        </div>
...
if (isset($endmsg['header'])) {
    echo "<h4>{$endmsg['header']}</h4>\n";
}
if (isset($endmsg['link'])) {
    echo "<a href='{$endmsg['link']}'>{$endmsg['prompt']}</a>\n";
}
?>
...
filtertoggle();
<?php if ($quickfind) {
    echo "focusOnForm('needle');\n";
} else {
    echo "focusOnForm('notcategory');\n";
} ?>
/* ]]> */ </script>
