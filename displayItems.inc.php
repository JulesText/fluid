...
if (basename($_SERVER['SCRIPT_FILENAME']) == 'reportContext.php' || basename($_SERVER['SCRIPT_NAME']) == 'reportContext.php') {
    $reportContext = true;
    $ran = rand(1, count($maintable));
    // echo $ran . " of " . count($maintable); die();
    // echo "<pre>"; var_dump($maintable);
} else {
    $reportContext = false;
}
...
foreach ($maintable as $row) {
    ...
    echo "<tr";
    if (!empty($row['row.class']) && $row['row.class'] != '') {
        echo " class='{$row['row.class']}' ";
    }
    echo ">\n";
        ...
        if ($show[$key]) {
            ...
            switch ($key) {
                case 'title':
                                ...
                                foreach ($result as $child) {
                                    ...
                                    if ($child['NA']) {
                                        if (!in_array($child['itemId'], $linkedItems, true)) {
                                            ...
                                            if ($val == 'a') {
                                                $childString .= '<span style="opacity: 0.5"><br>SEE ACTIONS</span>';
                                            }
                                            if ($val == 'w') {
                                                $childString .= '<span style="opacity: 0.5"><br>SEE WAITING ONS</span>';
                                            }
                                        }
                    ...
                    ,"' href='";
                    if (empty($row['doreport']) && $isProject) {
                        echo "itemReport.php?itemId={$row['itemId']}'";
                    } elseif (empty($row['doreport']) && !$isProject) {
                        echo "item.php?itemId={$row['itemId']}'";
                    } elseif ($row['doreport'] == 'item') {
                        echo "item.php?itemId={$row['itemId']}'";
                    } elseif ($row['doreport'] == 'parent') {
                        echo "itemReport.php?itemId={$row['itemId']}'";
                    } elseif ($row['doreport'] == 'cl') {
                        echo "reportLists.php?listId={$row['itemId']}&type=c'";
                    } elseif ($row['doreport'] == 'cli') {
                        echo "editListItems.php?itemId={$row['itemId']}&type=c'";
                    } elseif ($row['doreport'] == 'l') {
                        echo "reportLists.php?listId={$row['itemId']}&type=l'";
                    } elseif ($row['doreport'] == 'li') {
                        echo "editListItems.php?itemId={$row['itemId']}&type=l'";
                    } elseif ($row['doreport'] == 'fi') {
                        echo "fi.php?chat_id={$row['itemId']}'";
                    }
                    echo ">$cleaned</a>";

                    echo $childString;
                    if ($hasTickler) {
                        echo '<span style="opacity: 0.5"><br>TICKLED ITEM(S)</span>';
                    }
            ...
            /*
                    ...
                    if ($reportContext && $row['randomChoice']) {
                        echo '<br><span style="opacity: .5">r = ' . $row["runningCount"] . '</span>';
                    }
                    // else echo "<br>" . $row['runningCount'];
    ...
    */
    ...
    //                    echo "<a href='editCat.php?field=category&amp;id=",$row[$key.'Id'],"' title='Edit the {$row[$key]} category'>{$row[$key]}</a>";
                ...
                case 'parent':
                    if (empty($row[$key . 'Id'])) {
                        ...
                        foreach ($pids as $pkey => $pid) {
                            echo "$brk";
                            if (isset($ptypes[$pkey]) && strlen($ptypes[$pkey]) > 0) {
                                echo "<span style='opacity: 0.5'>" , strtoupper(makeclean($ptypes[$pkey])),":</span>";
                            }
                ...
                case 'context':
                    if ($row[$key] == '') {
                        echo '&nbsp;';
                    } else { //                    echo "<a href='reportContext.php#c",$row[$key.'Id'],"' title='Go to the ",$row[$key]," context report'>{$row[$key]}</a>";
                        echo "{$row[$key]}";
                    }
                    break;
                case 'spatialcontext':
                    if ($row[$key] == '') {
                        echo '&nbsp;';
                    } else { //                    echo "<a href='editCat.php?field=context&amp;id=",$row[$key.'Id'],"' title='Go to the ",$row[$key]," context report'>{$row[$key]}</a>";
                        echo "{$row[$key]}";
                    }
                    break;
                case 'timeframe':
