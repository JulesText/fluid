...
function gtd_searchdiv_init(ids, titles, types, onetype) {
  ...
  if (!useTypes) {
    qtype = thistype = mapTypeToName(onetype);
  }
  ...
  for (var i = 0; i < titles.length; i++) {
    ...
    if (useTypes) {
      thistype = mapTypeToName(types[i]);
    }
...
function gtd_gotparent(id, title, type) {
  var myargs;
  if (document.getElementById("parentrow" + id)) {
    return;
  }
