...
if ( $current > 0.83 || $current < 0.00 ) { // new
  header('Location:reportLists.php?listId=46&type=C');
}
if ( $current > 0.00 && $current < 0.20 ) { // wax crescent
  header('Location:reportLists.php?listId=47&type=C');
}
if ( $current > 0.20 && $current < 0.40 ) { // wax gibbous
  header('Location:reportLists.php?listId=48&type=C');
}
if ( $current > 0.40 && $current < 0.50 ) { // full
  header('Location:reportLists.php?listId=49&type=C');
}
if ( $current > 0.50 && $current < 0.70 ) { // wane gibbous
  header('Location:reportLists.php?listId=50&type=C');
}
if ( $current > 0.70 && $current < 0.83 ) { // wane crescent
  header('Location:reportLists.php?listId=51&type=C');
}
