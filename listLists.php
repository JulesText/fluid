...
if ($display) {
    $sort["getchecklists"] = "`sortBy` ASC, `title` ASC";
}

$cashtml = categoryselectbox($config, $values, $sort);
$values['filterquery'] = "";
if ($values['categoryId']) {
    $values['filterquery'] = " WHERE " . sqlparts("listcategoryfilter", $config, $values);
}
if ($values['catcodeId']) {
    $values['filterquery'] = " WHERE " . sqlparts("listcatcodefilter", $config, $values);
}
...
<?php if ($result) {
    ...
    <table class="datatable sortable" id="categorytable" summary="table of categories">
        ...
        <tbody><?php foreach ($result as $row) { ?>
            <tr>
                <td style="text-align: center" <?php if (!$display) {
                    echo 'class="togglehidden"';
                                               } ?>>
                <?php
                ...
                <td class="JKSmallPadding<?php if (!$display) {
                    echo ' togglehidden';
                                         } ?>"><?php
...
?></td>
                ...
                <td class="JKSmallPadding<?php if (!$display) {
                    echo ' togglehidden';
                     ...
                     $row['instanceId'] = '';
if ($row['scored'] == 'y') {
    echo scoreCL($config, $row, $sort);
} else {
    echo 'not scored';
}
?></td>

                <td class="<?php if ($display) {
                    echo "JKSmallPadding whitespace";
                           } else {
                               echo "togglehidden";
                           } ?>">
                    <?php
