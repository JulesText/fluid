...
if (!$values['itemId']) {
    ...
    if ($show['deadline'] && !empty($_GET['deadline'])) {
        $values['deadline'] = $_GET['deadline'];
    }
...
<form action="processItems.php" method="post" onsubmit="return validate(this);"><div class='form'>
    <div class='formrow'><span class="error" id='errorMessage'></span></div>
        ...
        if ($values['isTrade'] == 'y' && in_array($values['type'], ['a','w','r'])) {
            $values['tradeDisplay'] = true;
        } else {
            $values['tradeDisplay'] = false;
        }
        ...
        ?><div class='formrow'>
            <?php if ($show['category']) { ?>
                ...
                if ($values['itemId']) {
                    echo ajaxUpd('itemCategory', $values['itemId']);
                }
                ?>>
                <?php echo $cashtml; ?>
                </select>
            <?php } else {
                $hiddenvars['categoryId'] = $values['categoryId'];
            }
            if ($show['context']) { ?>
                <label for='context' class=''>Context:</label>
                <select name='contextId' id='context' <?php
                if ($values['itemId']) {
                    echo ajaxUpd('itemContext', $values['itemId']);
                }
                ?>>
                <?php echo $cshtml; ?>
                </select>
            <?php } else {
                $hiddenvars['contextId'] = $values['contextId'];
            }
            if ($show['timeframe'] && $values['type'] != 'p') { ?>
                <label for='timeframe' class=''>Time:</label>
                <select name='timeframeId' id='timeframe' <?php
                if ($values['itemId']) {
                    echo ajaxUpd('itemTime', $values['itemId']);
                }
                ?>>
                <?php echo $tshtml; ?>
                </select>
            <?php } else {
                $hiddenvars['timeframeId'] = $values['timeframeId'];
            } ?>
            <?php if ($show['NA']) { ?>
                <label for='nextAction' class=''>Next Action:</label><input type="checkbox" name="nextAction" id="nextAction" value="y" <?php
                if ($nextaction) {
                    echo " checked='checked' class='checked' ";
                } else {
                    echo " class='unchecked' ";
                }
                if ($values['itemId']) {
                    echo ajaxUpd('itemNA', $values['itemId']);
                }
                ?> />
            <?php } else {
                $hiddenvars['nextAction'] = ($nextaction) ? 'y' : '';
            } ?>
            ...
            if ($values['itemId']) {
                echo ajaxUpd('itemTrade', $values['itemId']);
            }
            echo "/>";
            ?>
        </div>
        ...
        <?php if ($show['dateCreated'] && $values['tradeDisplay']) { ?>
            <div class='formrow'>
                ...
                if ($values['itemId']) {
                    echo ajaxUpd('itemDateCreated', $values['itemId']);
                }
                ?>/>
        ...
        <?php } else {
            $hiddenvars['dateCreated'] = $values['dateCreated'];
        } ?>
        ...
        if ($show['tradeCondition'] && $values['tradeDisplay']) { ?>
          ...
          <select name='tradeConditionId' id='tradeCondition' onChange='toggleTradeCondition()' <?php
            if ($values['itemId']) {
                echo ajaxUpd('itemTradeCondition', $values['itemId']);
            }
            ?>>
            <?php echo $tcshtml; ?>
            </select>* required
        <?php } else {
            $hiddenvars['tradeConditionId'] = $values['tradeConditionId'];
        } ?>
        </div>
        ...
        if ($show['title']) { ?>
            <div class='formrow'>
                    ...
                    if ($values['itemId']) {
                        echo ajaxUpd('itemTitle', $values['itemId']);
                    }
                    ?> />
            </div>
        <?php } else {
            $hiddenvars['title'] = $values['title'];
        }
        ...
        if ($show['description']) { ?>
            ...
            ?>>
                    ...
                    <?php # item being edited (has itemId) not created so allow ajax save
                    if ($values['itemId']) {
                        echo ajaxUpd('itemDescription', $values['itemId']);
                    }
                    ?>><?php echo makeclean($values['description']); ?></textarea>
            </div>
        <?php } else {
            $hiddenvars['description'] = $values['description'];
        }
        ...
        if ($show['conclusion']) { ?>
            ...
            if ($values['isTrade'] === 'y') {
                ...
                if ($values['tradeConditionId'] == 0 && $values['tradeDisplay']) {
                    echo " style='display: none'";
                }
            }
            ?>>
        ...
        if ($show['behaviour']) { ?>
            ...
            $hiddenvars['behaviour'] = $values['behaviour'];
        }
        ...
        <div class='formrow'>
            <?php if ($show['deadline']) { ?>
                ...
                if ($values['itemId']) {
                    echo ajaxUpd('itemDeadline', $values['itemId']);
                }
                ?>/>
            ...
            <?php } else {
                $hiddenvars['deadline'] = $values['deadline'];
            }
            if ($show['dateCompleted']) { ?>
                ...
                if ($values['itemId'] && strlen($values['dateCompleted']) < 1) {
                    echo ajaxUpd('itemCompletedNow', $values['itemId']);
                }
                if ($values['itemId'] && strlen($values['dateCompleted']) > 1) {
                    echo ajaxUpd('itemCompletedEdit', $values['itemId']);
                }
                ?>/>
                <button type='reset' id='dateCompleted_trigger' <?php if (strlen($values['dateCompleted']) < 1) {
                    echo 'hidden';
                                                                } ?>>...</button>
                    <script type='text/javascript'>
                        Calendar.setup({
                ...
                <button type='button' id='dateCompleted_today' <?php if (strlen($values['dateCompleted']) > 1) {
                    echo 'hidden';
                                                               } ?> onclick="javascript:completeToday('dateCompleted');focusOnForm('dateCompleted');">Today</button>
            <?php } else {
                $hiddenvars['dateCompleted'] = $values['dateCompleted'];
            } ?>
        </div>
        <?php if ($show['repeat']) { ?>
        <div class='formrow'>
        ...
        <?php } else {
            $hiddenvars['repeat'] = $values['repeat'];
        } ?>
        <div class='formrow'>
            ...
            if (/*$show['isSomeday']*/ 1) {
                ...
                if ($values['itemId']) {
                    echo ajaxUpd('itemSomeday', $values['itemId']);
                }
                ?> />
            <?php } else {
                $hiddenvars['isSomeday'] = $values['isSomeday'];
            } ?>
        <!-- </div> -->
        ...
        if (/*$show['suppress']*/ 1) { ?>
            <!-- <div class='formrow'> -->
                                ...
                                if ($values['suppress'] == "y") {
                                    echo " checked='checked' class='checked'";
                                } else {
                                    echo " class = 'unchecked'";
                                }
                                if ($values['itemId']) {
                                    echo ajaxUpd('itemTickler', $values['itemId']);
                                }
                                ?>/>
                ...
                if ($values['itemId']) {
                    echo ajaxUpd('itemTicklerDays', $values['itemId']);
                }
                ?> /><label for='suppressUntil'>&nbsp;days before due date</label>
                                ...
                                if ($values['suppressIsDeadline'] == "y") {
                                    echo " checked='checked' class='checked'";
                                } else {
                                    echo " class='unchecked'";
                                }
                                if ($values['itemId']) {
                                    echo ajaxUpd('itemTicklerDeadline', $values['itemId']);
                                }
                                ?>/>
                        </div>
        <?php } else {
        ...
        if ($show['metaphor']) {
            ...
            $hiddenvars['metaphor'] = $values['metaphor'];
        }
        ?>
        ...
        <div class='formrow'>
            ...
            if ($values['itemId']) {
                echo ajaxUpd('itemLink', $values['itemId']);
            }
            ?> />
        </div>
        ...
        foreach ($hiddenvars as $key => $val) {
            echo hidePostVar($key, $val);
        }
