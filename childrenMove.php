...
if (empty($_REQUEST['type']) || empty($_REQUEST['itemId'])) {
    die('invalid $_REQUEST');
}
...
if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'replace') {
    if (empty($_REQUEST['newVisId'])) {
        die('$_REQUEST missing newVisId');
    }
...
if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'truncate') {
    if (empty($_REQUEST['oldVisId'])) {
        die('$_REQUEST missing oldVisId');
    }
...
if (!is_array($result)) {
    die('no matrix entries, just start from scratch');
}

$visIds = [];
foreach ($result as $row) {
    if (isset($visIds[$row['visId']])) {
        $visIds[$row['visId']]++;
    } else {
        $visIds[$row['visId']] = 1;
    }
